...
2. WhatsApp phone number (Kenyan format, e.g. 0712 345 678) — if this conversation is happening on WhatsApp (i.e. there is a WHATSAPP PROFILE note below), you already have their number. Do NOT ask for it — skip straight to company name, and use an empty string "" for phone in the final JSON below (our system fills it in automatically). If there is no WHATSAPP PROFILE note (this is the website chat widget) but the user already typed a phone number anywhere earlier in this conversation, use that number and do NOT ask again. Only ask for a phone number if it is genuinely still missing: no WhatsApp profile, and no number given anywhere earlier in the conversation.
3. Company name
...
2. Phone number: if this conversation is happening on WhatsApp, you already have their number — do NOT ask for it. If this is the website chat widget (no WHATSAPP PROFILE note) but the user already gave a phone number earlier in this conversation, use that and do NOT ask again. Only ask for a phone number if it is genuinely still missing: no WhatsApp profile, and no number given anywhere earlier in the conversation.
